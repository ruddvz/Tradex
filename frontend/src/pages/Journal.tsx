...
const emotionEmojis: Record<string, string> = {
  Confident: '💪', Focused: '🎯', Calm: '😌', Anxious: '😰',
  Fearful: '😨', Greedy: '💰', FOMO: '😱', Revenge: '😤',
  Neutral: '😐', Excited: '🤩', Patient: '🧘',
};
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-surface-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-xs text-slate-400">{format(new Date(trade.entryTime), 'MMM dd, yyyy · h:mm a')}</span>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface-light text-slate-400 hover:text-white transition-colors">✕</button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className={clsx(
            ...
            trade.pnl >= 0 ? 'bg-brand-500/10 border border-brand-500/20' : 'bg-red-500/10 border border-red-500/20'
          )}>
            <div className={clsx('text-3xl font-bold', trade.pnl >= 0 ? 'text-brand-400' : 'text-red-400')}>
              {trade.pnl >= 0 ? '+' : ''}${trade.pnl.toFixed(2)}
            </div>
            <div className="text-sm text-slate-400 mt-1">
              {trade.rMultiple >= 0 ? '+' : ''}{trade.rMultiple}R · {trade.pnl >= 0 ? '+' : ''}{trade.pnlPercent}%
            </div>
          </div>
          ...
          {trade.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {trade.tags.map(tag => (
                <span key={tag} className="px-2 py-1 bg-surface-light rounded-lg text-xs text-slate-400 border border-surface-border">
                  #{tag}
                </span>
              ))}
            </div>
...
export function Journal() {
  ...
  const filtered = useMemo(() => trades.filter(t => {
    ...
    return true;
  }), [trades, search, symbolFilter, statusFilter]);
  ...
  return (
    <div className="min-h-screen">
      <Header title="Trade Journal" subtitle={`${trades.length} trades recorded`} />
      ...
      <div className="pt-16 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Net P&L', value: `${totalPnl >= 0 ? '+' : ''}$${totalPnl.toFixed(0)}`, sub: 'filtered' },
            { label: 'Win Rate', value: filtered.length ? `${((wins / filtered.length) * 100).toFixed(0)}%` : '—', sub: `${wins}W / ${filtered.length - wins}L` },
            { label: 'Avg P&L', value: filtered.length ? `$${(totalPnl / filtered.length).toFixed(0)}` : '—', sub: 'per trade' },
          ].map(s => (
            <div key={s.label} className="card p-4">
              <div className="text-xs text-slate-500 mb-1">{s.label}</div>
              <div className={clsx('text-xl font-bold', s.label === 'Net P&L' ? (totalPnl >= 0 ? 'text-brand-400' : 'text-red-400') : 'text-white')}>{s.value}</div>
                ...
                {filtered.map(trade => (
                  ...
                  >
                    ...
                    <td className="table-cell hidden xl:table-cell">
                      <Badge variant="neutral" size="xs">{trade.session}</Badge>
                    </td>
                    <td className="table-cell hidden lg:table-cell">
