...
function formatChartDate(date: string) {
  try {
    return date.includes('T') ? format(parseISO(date), 'MMM dd') : format(new Date(date), 'MMM dd');
  } catch {
    return date;
  }
}
...
export function EquityCurve({ showDrawdown = false, height = 220, showSourceBadge = false }: EquityCurveProps) {
  ...
  return (
    <div>
      ...
      {data.length === 0 ? (
        <p className="text-sm text-text-muted py-8 text-center">No equity data for this range.</p>
      ) : (
