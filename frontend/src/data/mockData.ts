...
export function generateTrades(count = 120): Trade[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    trades.push({
      ...
      takeProfit:
        direction === 'BUY' ? entryPrice + priceRange * 0.5 : entryPrice - priceRange * 0.5,
      strategy,
      session,
      emotion,
      ...
      notes:
        i % 4 === 0
          ? `Clean setup on ${symbol}. ${isWin ? 'Followed plan perfectly.' : 'Exited early due to volatility.'}`
          : '',
...
export const mockPlaybooks: Playbook[] = [
  ...
  },
  {
    ...
    rules: [
      'Trade only 08:00-10:00 GMT',
      'Look for Asia range liquidity sweep',
  ...
  },
  {
    ...
    rules: [
      'Trade 13:30-15:00 GMT only',
      'Must have news catalyst',
      'Max 30 minute hold time',
      'Hard SL always in place',
    ],
    tags: ['scalp', 'new-york', 'volatility'],
    performance: Array.from({ length: 20 }, (_, i) => ({
      date: format(subDays(new Date(), 20 - i), 'yyyy-MM-dd'),
      pnl: Math.random() * 400 - 120,
    })),
  ...
  },
  {
    ...
    rules: [
      'Only D1/W1 timeframe levels',
      'Zone must be fresh (untested)',
      'Confirmation on H4/H1',
      'Target minimum next S/D zone',
    ],
    tags: ['supply-demand', 'swing', 'weekly'],
    performance: Array.from({ length: 20 }, (_, i) => ({
      date: format(subDays(new Date(), 20 - i), 'yyyy-MM-dd'),
      pnl: Math.random() * 700 - 100,
    })),
...
export const mockNotebook: NotebookEntry[] = [
  {
    id: 'note-1',
    title: 'Trading Rules - Core Principles',
    type: 'rule',
...
- Process over outcome`,
    tags: ['psychology', 'mindset', 'lessons'],
    pinned: false,
    createdAt: format(subDays(new Date(), 30), 'yyyy-MM-dd'),
    updatedAt: format(subDays(new Date(), 5), 'yyyy-MM-dd'),
  },
...
export const mockAIInsights: AIInsight[] = [
  {
    ...
    description:
      'Analysis of 120 trades shows you perform 43% better on Tuesdays vs other days. Win rate: 74% vs average 65%. Your best setups align with Tuesday institutional flow.',
  ...
  },
  {
    ...
    description:
      'After 2 consecutive losses, your next trade win rate drops to 41%. You show signs of revenge trading — larger lot sizes and faster entries within 30 minutes.',
  ...
  },
  {
    ...
    description:
      'Your XAUUSD trades during London open (08:00-10:00 GMT) show a 71% win rate and 4.1 profit factor — significantly above your overall average.',
  ...
  },
  {
    ...
    description:
      'Trades tagged "FOMO" or "Anxious" result in 23% win rate. Trades tagged "Calm" or "Patient" show 78% win rate. Your emotional state is your biggest performance variable.',
  ...
  },
  {
    ...
    description:
      'You just completed your longest win streak of 10 consecutive profitable trades. Your average RR during this streak was 3.2.',
